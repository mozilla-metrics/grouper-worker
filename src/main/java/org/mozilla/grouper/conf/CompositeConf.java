package org.mozilla.grouper.conf;
...
class CompositeConf implements Conf {
  ...
  String get(String key) {
    if (upper_.get(key) != null) return upper_.get(key);
    if (lower_.get(key) != null) return lower_.get(key);
  ...
  @Override public <C>
  List<C> getList(Class<C> type, String key) {
  ...
  static private class Bottom implements Conf {
    ...
    @Override public <C>
    List<C> getList(Class<C> valueType, String type) {
      return null;
    }
