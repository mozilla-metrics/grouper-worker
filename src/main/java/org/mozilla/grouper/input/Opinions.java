...
class Opinions implements Iterable<String[]> {
  ...
  static
  enum Field {
    ID(0), TIMESTAMP(1), TYPE(2), PRODUCT(3), VERSION(4), PLATFORM(5),
    LOCALE(6), MANUFACTURER(7), DEVICE(8), URL(9), TEXT(10);
    public int i;
    Field(int c) { i = c; }
  }
  ...
  private
  class OpinionsIterator implements Iterator<String[]> {
    ...
    public
    OpinionsIterator(TsvReader reader) {
      reader_ = reader;
    }
    ...
    @Override public
    boolean hasNext() {
      if (row_ != null) return true;
      try {
        row_ = reader_.nextRow();
        if (row_ == null) return false;
