...
public class Assert {
  ...
  public static
  void nonNull(Object... values) {
    int i = 0;
    for (Object value : values) {
      ++i;
      if (value == null) {
        String message = String.format("%s Value %d/%d is null.",
                                       PREFIX, i, values.length);
  ...
  public static
  void check(boolean... values) {
    int i = 0;
    for (boolean value : values) {
      ++i;
      if (!value) {
        String msg = String.format("%s Check %d/%d failed!",
                                   PREFIX, i, values.length);
  ...
  public static
  void unreachable(String message, Object... objects) {
  ...
  public static <T>
  T unreachable(Class<T> returnType) {
  ...
  /** @see #unreachable(Class) */
  public static <T>
  T unreachable(Class<T> returnType, Exception problem) {
    String msg = String.format("%s Code should be unreachable\n", PREFIX);
