...
/**
 * Reads all model objects of a type from HBase.
 * We'll add filter criteria as needed.
 */
public class Source<T extends Model> implements Iterable<T> {
  ...
  Iterator<T> iterator() {
    try {
      return new ModelIterator<T>(Adapters.create(factory_, model_),
                                 table_.getScanner(new Scan()));
    }
    catch (IOException e) {
      e.printStackTrace();
  ...
  static
  class ModelIterator<S extends Model> implements Iterator<S> {
    ...
    @Override public
    S next() {
      if (!it_.hasNext()) throw new NoSuchElementException("Scanner empty!");
