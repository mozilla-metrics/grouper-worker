...
public class Importer<T extends Model> {
  ...
  public
  Importer(final Factory factory, final Class<T> model) {
    model_ = model;
    factory_ = factory;
  ...
  public
  void load(T item) throws IOException {
  ...
  /**
   * So there is this factory where all workers do is running and then relax at
   * the pool, and where all clients must wait in a queue.
   * It is a pretty fun work environment... until everyone gets garbage
   * collected that is.
   */
  private
  ExecutorService workers() {
    return new ThreadPoolExecutor(
        10, 20, 90, TimeUnit.SECONDS,
  ...
  private
  void shutdownGracefully(final ExecutorService pool) {
    pool.shutdown();
    try {
  ...
  private final ThreadLocal<HTableInterface> table_ =
   ...
   * to release the HTables in long running programs:
   * To this end, when spawning a worker, the thread pool adds a reference to
   * this list, so the HTables used by the workers can be released upon
   * completion.
   */
  final List<HTableInterface> tables_ =
    new java.util.LinkedList<HTableInterface>();
